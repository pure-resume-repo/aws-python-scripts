...
def main():
    ...
    if len(asg_results["AutoScalingGroups"]) == 0:
        print(f"NO results found for asg_name {asg_name}. \nResult: {asg_results}")
        exit()
    ...
    if asg["NewInstancesProtectedFromScaleIn"] is False:
        print("NewInstancesProtectedFromScaleIn is currently False, setting to True")
        autoscaling.update_auto_scaling_group(
            AutoScalingGroupName=asg_name, NewInstancesProtectedFromScaleIn=True
        )
